...
//       const storedUserId = await AsyncStorage.getItem("userId"); // Fetch vehicle owner ID
//       setUserId(storedUserId);
//     };

//     fetchUser();

//     socket.emit("joinChat", { userId, mechanicId });
...
// const API_URL2 = "http://10.0.2.:5000/messages"; 
...
// const ChatScreen: React.FC = () => {
//   const route = useRoute();
//   const { mechanicId, mechanicName,chatId } = route.params as { mechanicId: string; mechanicName: string ; chatId: string;};
//   // const { chatId, mechanicId } = route.params as { chatId: string; mechanicId: string; mechanicName: string };
//   const [userId, setUserId] = useState<string | null>(null);
//   const [messages, setMessages] = useState<Message[]>([]);
//   const [newMessage, setNewMessage] = useState("");
...
//         const response = await axios.get(`${API_URL}/api/vehicleOwner/profile`, {
...
//         if (!fetchedUserId) {
//           console.log("Vehicle Owner ID not found in response");
//           return;
//         }

//         console.log("Fetched Vehicle Owner ID:", fetchedUserId);
//         setUserId(fetchedUserId);

//         // Emit joinChat only after userId is fetched
...
//       socket.off("receiveMessage");
//     };
//   }, [mechanicId]); // No `userId` in dependencies to prevent unnecessary re-renders
...
// const response = await axios.get(`${API_URL2}/${chatId}`);
